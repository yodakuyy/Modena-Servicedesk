        ...
        <div className="p-6 border-b border-gray-50 flex justify-between items-start">
           ...
           <div className="flex items-center gap-3 flex-1 justify-end max-w-3xl">
               ...
               <div className="relative" ref={filterRef}>
                 ...
                 {/* Filter Dropdown */}
                 {showFilter && (
                    <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-100 z-20 p-5 animate-in fade-in zoom-in-95 duration-100">
                           ...
                           <div className="flex flex-wrap gap-2">
                             {['Open', 'Pending', 'Resolved', 'Closed'].map(status => (
                               <button 
                                 ...
                                 className={`px-2 py-1 text-[10px] rounded-md border font-medium transition-colors ${selectedStatuses.includes(status) ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                               >
                                 {status}
                               </button>
                             ))}
                        ...
                        <div className="space-y-1.5">
                           ...
                           <div className="flex flex-wrap gap-2">
                             {['Running', 'Stopped'].map(status => (
                               <button 
                                 key={status}
                                 onClick={() => toggleFilter(selectedSla, status, setSelectedSla)}
                                 className={`px-2 py-1 text-[10px] rounded-md border font-medium transition-colors ${selectedSla.includes(status) ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                               >
                                 {status}
                               </button>
                             ))}
