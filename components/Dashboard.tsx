...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, active = false, badge = '' }) => (
  <div className={`flex items-center justify-between px-6 py-3 cursor-pointer border-l-4 transition-colors ${active ? 'bg-indigo-50 border-indigo-600' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}>
    ...
    <div className="flex min-h-screen bg-[#f3f4f6] font-sans">
      ...
      <div className="w-64 bg-white border-r border-gray-200 flex flex-col hidden lg:flex sticky top-0 h-screen">
        ...
        <nav className="flex-1 mt-6 space-y-1">
          ...
          <SidebarItem icon={Ticket} label="Tickets" badge="39" />
          <SidebarItem icon={Package} label="Inventory" />
          <SidebarItem icon={Book} label="Knowledge Base" />
          <SidebarItem icon={FolderKanban} label="Projects" badge="4" />
          <SidebarItem icon={Settings} label="Settings" />
        </nav>

        <div className="p-6">
          <button className="w-full bg-indigo-600 text-white rounded-lg py-3 flex items-center justify-center gap-2 font-medium hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200/50">
            <Plus size={20} />
            Create a Ticket
          </button>
        </div>
        ...
        <div className="p-6 border-t border-gray-100 flex items-center gap-3">
          <img src="https://ui-avatars.com/api/?name=Mary+Wells&background=random" alt="User" className="w-10 h-10 rounded-full border-2 border-white shadow-sm" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-gray-700 truncate">Mary Wells.45</p>
          </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* Header */}
        <header className="h-20 bg-white/80 backdrop-blur-sm flex items-center justify-between px-8 z-10">
          <div className="flex items-center gap-2 bg-white rounded-lg px-4 py-2 shadow-sm border border-gray-100 max-w-2xl">
             <span className="font-bold text-gray-800 text-sm whitespace-nowrap">Kudos</span>
             <Star className="w-4 h-4 text-purple-500 fill-current flex-shrink-0" />
             <div className="text-sm text-gray-500 truncate">
                so nice & helpful! Thank you! - Martha Stephens &nbsp;&nbsp; ★ &nbsp;&nbsp; Thanks for taking care of that so quickly!
             </div>
          </div>
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="Search" 
              className="pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm w-72 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 outline-none transition-all shadow-sm"
            />
          </div>
        </header>
